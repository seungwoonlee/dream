...
const int MAX_VARIABLE_NAME_LENGTH = 64;	// 최대 변수이름 길이
...
void  ProcessCommand(char* str, char varName[MAX_VARIABLE_COUNT][MAX_VARIABLE_NAME_LENGTH], int varValue[MAX_VARIABLE_COUNT]);
...
int DetectStringType(char* str)
{
	...
	{
		if (zStrstr(str, "print ") || zStrstr(str, "view") || zStrstr(str, "quit") || zStrstr(str, "quit"))
		{
...
void ProcessCommand(char* str, char varName[MAX_VARIABLE_COUNT][MAX_VARIABLE_NAME_LENGTH], int varValue[MAX_VARIABLE_COUNT])
{
	...
	const int CMD_VIEW = 2;
	const int CMD_EXIT  = 3;
	
	int status = CMD_NOTHING;
	char buf[MAX_BUF2] = { 0, };
	char strVar[MAX_VARIABLE_NAME_LENGTH] = { 0, };
	char strVarPrint[MAX_BUF2] = { 0, };
	int cnt = 0;
	int i = 0;
	...
	else if (status == CMD_VIEW)
	{
		while (varName[i][0])
		{
			PutS(varName[i]);
			PutS("=");
			zItoa(GetVariableValue(varName[i], varName, varValue), strVarPrint);
			PutS(strVarPrint);
			PutS("\n");
			i++;
		}
	...
	else if (status == CMD_EXIT)
	{
		PutS("Quitting...\n");
		return;
	}
	if (!CheckAlreadyDeclare(strVar, varName))
	{
		PutS("the Variable was not declared\n");
		return;
	}
	PutS(strVar);
	PutS("=");
	zItoa(GetVariableValue(strVar, varName, varValue), strVarPrint);
	PutS(strVarPrint);
	PutS("\n");
	return;
}
...
void ProcessEquation(char* str, char varName[MAX_VARIABLE_COUNT][MAX_VARIABLE_NAME_LENGTH], int varValue[MAX_VARIABLE_COUNT])
{
	...
	RemoveSpace(buf, str);
	posEq = zStrnchr(buf, '=');
	posSemi = zStrnchr(buf, ';');

	zStrncpy(leftSide, buf, posEq);
	zStrncpy(rightSide, buf+posEq+1, posSemi-posEq-1);

	while (1)
	{ // rightSide 에 있는 모든 변수가 없어질때까지 변수값으로 치환
		if (!HasAlpha(rightSide)) // 수식에 알파벳이 없으면
		{
			break;
		}
...
//	int temp;
	...
	length = zStrlen(str);
	posSemi = zStrnchr(str, ';');

	zStrncat(buf, str+4, posSemi-4);
...
void main(void)
{
	...
	PutS("> Available Command :  print [Variable] , <Enter>  without ';'\n");
	PutS("> Language Sentence :  [Declaration] | [Equation]     with ';'\n> \n");
	...
	while (1)
	{
		...
		case TYPE_STRING_COMMAND:
			ProcessCommand(buf, varName, varValue);		// 콘솔명령어 모드 변수명과 변수에 현재 저장된 값을 보여준다.
			break;
