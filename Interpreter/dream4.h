...
const int MAX_BUF2 = 1000;	// 1000 으로 개발하면 너무 커서 디버그가 불편하므로 임시로 쓴다.
...
char* zStrreplaceWhole(char* dst, char* key, char* add); // dst 에서 key와 동일한 단어를 찾아서 add로 바꿔치기 한다.
int   zAtoi(const char *str);
char* zItoa(int num, char* str);
int   zIsAlpha(int c);
int   zIsDigit(int c);
int   zIsAlnum(int c);
int   zIsDelimiter(int c);	// 구분자  +,-,*,/,%,(,),",=,>,<,?,:,&,|,~,!,\0
int   zToLower(int c);
int   zToUpper(int c);
